...
//            // Listen for events
    ...
    private void upd(JSONObject message){
        ...
        x++;
        try {
            if (message.getString("chat_id").equals("0")){
                Log.d("new_chat_abcde", String.valueOf(x));
                new Thread(() -> {
                    try {
                        Thread.sleep(1000);
                ...
                new Thread(() -> {
                    ...
                    ChatActivity.update();
                    Log.d("abcde", "ChatActivity updated");

                    UsersActivity.update();
                    Log.d("abcde", "UserActivity updated");
    ...
    public void createChat(int activeId,String username){
        ...
        try {
            ...
            messageObject.put("socket", id);
            messageObject.put("username", username);
            messageObject.put("message", "new chat");
            socket.emit("send_message", messageObject);
        } catch (JSONException e) {
            throw new RuntimeException(e);
        }
    }
    public void sendMessage(int activeId, String username, String message){
        JSONObject messageObject = new JSONObject();
        try {
            messageObject.put("chat_id", activeId);
            messageObject.put("socket", id);
